...
class IdP(models.Model):
    ...
    email_auth_errors_to_admins = models.BooleanField(
        _("Email auth errors to admins"),
        default=True,
        help_text=_(
            "When enabled, failed SAML authentications are logged at error "
            "level (triggering Django ADMINS emails if configured)."
        ),
    )
    auth_failed_message = models.TextField(
        _("Authentication failed message"),
        blank=True,
        help_text=_(
            "Shown to a user who authenticates with the IdP but has no MyCE "
            "account. Leave blank to use the default message."
        ),
    )
    log_response_attributes = models.BooleanField(
        _("Log response attributes"),
        default=True,
        help_text=_(
            "When enabled, store one IdPAttributeLog row (all SAML attributes) "
            "for every response from this IdP. Disable for IdPs where retaining "
            "the attribute payload is undesirable."
        ),
    )
